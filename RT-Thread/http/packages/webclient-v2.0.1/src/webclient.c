                                     ...
                                     const char *url, char **request)
{
    ...
    /* resolve the host name. */
    {
        ...
        ret = 1; //lwip_getaddrinfo(session->host, port_str, &hint, res);
        if (ret != 0)
        {
            LOG_E("getaddrinfo err: %d '%s'.", ret, session->host);
            rc = -WEBCLIENT_ERROR;
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    {
        socket_handle = 1;//socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP);
        if (socket_handle < 0)
        {
        ...
        if (1/*connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0*/)
        {
            /* connect failed */
