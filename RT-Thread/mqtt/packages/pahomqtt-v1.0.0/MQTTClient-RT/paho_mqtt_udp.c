...
static int sendPacket(MQTTClient *c, int length)
{
    ...
    // Origin: rc = send(c->sock, c->buf, length, 0);
    rc = RTThread_HAL_FE_Out(length);
    if (rc == length)
    {
        rc = 0;
    }
...
static int net_read(MQTTClient *c, unsigned char *buf,  int len, int timeout)
{
    ...
    while (bytes < len)
    {
        // Origin: rc = recv(c->sock, &buf[bytes], (size_t)(len - bytes), MSG_DONTWAIT);
        rc = RTThread_HAL_FE_In(&buf[bytes], (size_t)len - bytes);
...
static int MQTTConnect(MQTTClient *c)
{
...

    {
        ...
        // Origin: res = select(c->sock + 1, &readset, RT_NULL, RT_NULL, &timeout);
        res = 1;
...
static int MQTTSubscribe(MQTTClient *c, const char *topicFilter, enum QoS qos)
{
...

    {
        ...
        res = 1; // Origin: select(c->sock + 1, &readset, RT_NULL, RT_NULL, &timeout);
...
static void paho_mqtt_thread(void *param)
{
    ...
    // Origin: c->pub_sock = socket(AF_INET, SOCK_DGRAM, 0);
    c->pub_sock = 666;
    if (c->pub_sock == -1)
    {
    ...
    /* bind publish socket. */
    {
        ...
        // Origin: rc = bind(c->pub_sock, (struct sockaddr *)&pub_server_addr, sizeof(struct sockaddr));
        rc = 1;
        if (rc == -1)
        {
...
_mqtt_start:
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; i++)
    {
        ...
        rc = 0; //MQTTSubscribe(c, topic, QOS2);
    ...
    while (1)
    {
        ...
        res = 1;//select(((c->pub_sock > c->sock) ? c->pub_sock : c->sock) + 1,
        ...
        if (res == 0)
        {
            ...
            res = 1;//select(c->sock + 1, &readset, RT_NULL, RT_NULL, &timeout);
            if (res <= 0)
            {
...
/*
        if (FD_ISSET(c->pub_sock, &readset))
        {
            ...
            // Origin: len = recvfrom(c->pub_sock, c->readbuf, c->readbuf_size, MSG_DONTWAIT,
            //               (struct sockaddr *)&pub_client_addr, &addr_len);
            len = RTThread_HAL_FE_In(c->readbuf, c->readbuf_size);
            if (len == -1) {
                printf("Finish\n");
                exit(0);
            }
           ...
           if (pub_client_addr.sin_addr.s_addr != *((uint32_t *)(&netif_default->ip_addr)))
            ...
            {
                c->readbuf[len] = '\0';
                debug_printf("pub_sock recv %d byte\n", len, c->readbuf);
...
static int MQTT_local_send(MQTTClient *c, const void *data, int len)
{
    ...
    // Origin: send_len = sendto(c->pub_sock, data, len, MSG_DONTWAIT,
    //                  (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
    send_len = RTThread_HAL_FE_Out(len);
